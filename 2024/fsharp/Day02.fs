...
module Day02 =
    ...
    let findUnsafeLevel (report: int[]) =
        let levelDiffs = report |> Seq.pairwise |> Seq.map (fun (a, b) -> a - b)
    ...
    let checkSafeWithSafetyDampener (report: int[]) : bool =
        ...
        | Some index ->
            [ -1; 0; 1 ]
            |> List.exists (fun delta ->
                (index + delta) >= 0 && (report |> Array.removeAt (index + delta) |> checkSafe))

    let run (input: byte[]) (output: int -> string -> unit) =
        let reports = input |> bsplit '\n'B |> Array.map parseInts

        reports |> Seq.filter checkSafe |> Seq.length |> string |> output 1
