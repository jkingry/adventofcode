...
public sealed class CalendarGenerator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(fullPipeline, static (productionContext, options) =>
        {
            ...
            productionContext.AddSource("AdventOfCode.Cli.AdventOfCodeBenchmarks.g", GenerateBenchmarkClass(rootNamespace, solutions));
        });
    }
    ...
    {
        public string Identifier => $"{Year}_{Day}_{MethodName}";
    }
    ...
    private static string GenerateBenchmarkClass(string rootNamespaceInput, IEnumerable<SolutionInfo> methods)
    {
        var rootNamespace = string.IsNullOrEmpty(rootNamespaceInput)
            ? string.Empty
            : $"\nnamespace {rootNamespaceInput};\n";
        
        var days = methods.Select(m => (m.Year, m.Day)).Distinct().ToList();

        var inputDeclarations = 
            string.Join("\n", days.Select(m => $"    private byte[]? _input{m.Year}_{m.Day};"));
        
        var inputSetups =
            string.Join("\n", days.Select(m => $$"""
                    var text{{m.Year}}_{{m.Day}} = _pole.Client.GetInputAsync(InputType.Official, {{m.Year}}, {{m.Day}}).Result;
                    _input{{m.Year}}_{{m.Day}} = System.Text.Encoding.UTF8.GetBytes(text{{m.Year}}_{{m.Day}}  ?? throw new InvalidOperationException("No input found for {{m.Year}}, day {{m.Day}}"));
            """));

        var baselineMethods = methods
            .GroupBy(m => (m.Year, m.Day))
            .Select(g => g.First())
            .ToList();
