...
type RunCommand() =
    ...
    override u.Execute(context, settings) =
        ...
        let year =
            if settings.Year > 0 then
                settings.Year
            else
                days |> List.map (fun d -> d.year) |> List.max

        let day =
            if settings.Day > 0 then
                settings.Day
            else
                days
                |> List.filter (fun d -> d.year = year)
                |> List.map (fun d -> d.day)
                |> List.max
...
type TestCommand() =
    ...
    override u.Execute(context, settings) =
        ...
        let targets =
            if settings.Targets = null || settings.Targets = Array.empty then
                [| "" |]
            else
                settings.Targets

        let selected = targets |> Array.fold (parseYearDays days) Set.empty

        let selectedDays =
            days |> List.filter (fun d -> selected |> Set.contains (d.year, d.day))
