...
module Day17 =    
    ...
    let countOn (p : char[,,]) =
        let mutable c = 0
        Array3D.iter (fun v -> if v = '#' then c <- c + 1) p
        c
    ...
    let countOn4 (p : char[,,,]) =
        let mutable c = 0
        Array4D_iter (fun v -> if v = '#' then c <- c + 1) p
        c

    let adj4 x y z w (p : char[,,,]) =
        let n = p[(x - 1)..(x + 1), (y - 1)..(y + 1), (z - 1)..(z + 1), (w - 1)..(w + 1)]
    ...
    let trim4 (p : char[,,,]) : char[,,,] =
        ...
        Array4D.init (1 + max[0] - min[0]) (1 + max[1] - min[1]) (1 + max[2] - min[2]) (1 + max[3] - min[3]) (fun x y z w -> p[x + min[0], y + min[1], z + min[2], w + min[3]])
