...
module Day22 =
    ...
    let parse (input: string) : Decks =
        ...
        input |> splitDoubleLine |> Array.map parsePlayer |> Map.ofArray
    ...
    let combatGame (round: Decks -> int) (mp: Decks) : int * int list =
        ...
        while Option.isNone gameWinner do
            if pastStates |> Set.contains state then
                gameWinner <- Some 1
            else
                ...
                if state[loser].Length = 0 then
                    gameWinner <- Some winner
