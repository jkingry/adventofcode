    ...
    module Grid =
        ...
        let set (x: int) (y: int) (v: 'T) (grid: grid<'T>) : grid<'T> =
            ...
            while ax < 0 || ay < 0 || ax >= sz || ay >= sz do
                ...
                g <-
                    Array2D.init sz' sz' (fun x y ->
                        if x >= d && y >= d && x < (sz + d) && y < (sz + d) then
                            g[x - d, y - d]
                        else
        ...
        let get (x: int) (y: int) (grid: grid<'T>) =
            ...
            if ax >= 0 && ay >= 0 && ax < sz && ay < sz then
                g[ax, ay]
            else
                Unchecked.defaultof<'T>

        let setWith (x: int) (y: int) (getValue: 'T -> 'T) (grid: grid<'T>) = set x y (get x y grid |> getValue) grid
        let copy (g, o) = (Array2D.copy g), o
            ...
            let (q, r, _) =
                ...
                |> Seq.fold
                    (fun (q, r, s) c ->
                        ...
                        | a -> failwith (sprintf "Unreachable %A" a))
                    (0, 0, 0)
