...
module Day08 =
    ...
    let run (input: byte[]) (output: int -> string -> unit) =
        ...
        let a =
            lines
            |> Array.map (fun s -> s.ToCharArray() |> Array.map (fun x -> (byte x) - '0'B))
            |> array2D
        ...
        let traverse x y v =
            ...
            if
                top |> Array.forall (fun vv -> vv < v)
                || bot |> Array.forall (fun vv -> vv < v)
                || rgt |> Array.forall (fun vv -> vv < v)
                || lft |> Array.forall (fun vv -> vv < v)
            then
                visible <- visible + 1
    ...
    type RightEntry = { col: int; rgt: int }

    type BottomEntry = { bot: int; score: int }
