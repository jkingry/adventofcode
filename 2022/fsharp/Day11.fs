...
module Day11 =
    ...
    let runFast (input: byte array) (output: int -> string -> unit) =
        ...
        let monkeys = monkeys |> Array.map (fun m -> 
        ...
        let turn short i (monkeys : FMonkey array) =
            let monkey = monkeys[i]
        ...
        let round short (monkeys : FMonkey array) = 
            for i=0 to (monkeys.Length - 1) do
                turn short i monkeys
                        
        let shortMonkeys = monkeys |> Array.map (fun m -> { m with items = m.items |> Array.copy })
        for _ = 1 to 20 do
            round true shortMonkeys
