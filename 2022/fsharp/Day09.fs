...
module Day09 =
    ...
    let follow (rx: int[], ry: int[]) (dir: byte, amt: int) =
        ...
        seq {
            for _ in 1..amt do
                ...
                if moved then
                    yield (Array.last rx), (Array.last ry)
        }

    let createRope len =
        (Array.zeroCreate len), (Array.zeroCreate len)
    ...
    let trackMotionCountHashSet ropeLength instructions =
        ...
        let o = instructions |> Seq.map (follow rope) |> Seq.concat
    ...
    let parseInstructionsFast (input: byte[]) =
        let newline = '\n'B
