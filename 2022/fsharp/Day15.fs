...
module Day15 =
    ...
    let parseFast (input: byte[]) =
        let mutable i = 0

        let mutable res = []

        while i < input.Length do
            let (i2, sx) = parseIntToAny input i
            let (i3, sy) = parseIntToAny input (i2 + ", y=".Length)
            let (i4, bx) = parseIntToAny input (i3 + ": closest beacon is at x=".Length)
            let (i5, by) = parseIntToAny input (i4 + ", y=".Length)

            res <- (sx, sy, (manhattanDistance sx sy bx by)) :: res
            i <- (i5 + "\nSensor at x=".Length)
    ...
    let run (input: byte[]) (output: int -> string -> unit) =
        ...
        let boundaries =
            [ (System.Int32.MinValue, 0); (maxBoundary, System.Int32.MaxValue) ]
    ...
    let findElf (minX, minY) (maxX, maxY) (readings: (int * int * int)[]) =
        ...
        while elf.IsNone && i < readings.Length do
            ...
            elf <-
                outsideBorder
                |> Seq.tryFind (fun (x, y) ->
                    readings |> Array.forall (fun (sx, sy, d) -> (manhattanDistance sx sy x y) > d))
