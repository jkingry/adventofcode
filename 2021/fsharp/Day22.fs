...
module Day22 =
    ...
    let isValid ((x1, y1, z1), (x2, y2, z2)) = x1 < x2 && y1 < y2 && z1 < z2

    let getVolume ((x1, y1, z1), (x2, y2, z2)) = (x2 - x1) * (y2 - y1) * (z2 - z1)
    ...
    let getOverlap (a: Cuboid) (b: Cuboid) =
        ...
        let overlap =
            ((max aulx bulx), (max auly buly), (max aulz bulz)), ((min abrx bbrx), (min abry bbry), (min abrz bbrz))

        if isValid overlap then Some overlap else None

    type Instruction = { value: bool; cube: Cuboid }
