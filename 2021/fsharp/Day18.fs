...
module Day18 =
    ...
    let reduce f =
        let rec explode depth f =
            match f with
            | FPair(FNumber a, FNumber b) when depth = 4 -> Some(a, FNumber 0, b)
            | FPair(a, b) ->
    ...
    let add a b = FPair(a, b)

    let rec magnitude f =
        match f with
        | FNumber n -> n |> int64
        | FPair(a, b) -> (3L * (magnitude a)) + (2L * (magnitude b))
    ...
    let run data (output: int -> string -> unit) =
        ...
        let rec ppair =
            between (pstring "[") (pstring "]") (tuple2 (fvalue .>> (pstring ",")) fvalue |>> FPair)

        fvalueRef.Value <- choice [ pnumber; ppair ]

        let list = input |> splitLine |> Array.map (runParser fvalue)

        let part1 = list |> Array.reduce (fun a b -> add a b |> reduce)
