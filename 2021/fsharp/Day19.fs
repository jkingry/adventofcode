...
module Day19 =
    ...
    let parseScan input =
        ...
        let beacons = lines |> Array.map (ints >> array2tuple3) |> Set.ofArray
    ...
    let matchScans admap aud bdmap bud =
        ...
        let samedistances = Set.intersect aud bud |> Seq.toList
        //printfn " Same Distances %d" (samedistances |> List.length)

        if samedistances |> List.length < 12 then
            None
        else
    ...
    let run input (output: int -> string -> unit) =
        ...
        let mutable scans = input |> splitDoubleLine |> Array.map parseScan |> Map.ofArray
        ...
        while scans |> Map.count > 1 do
            ...
            scans <- scans |> Map.add an ab_beacons |> Map.remove bn
            distances <- distances |> Map.add an ab_distmap |> Map.remove bn
            uniquedistances <- uniquedistances |> Map.add an (toUniqueDistances ab_distmap) |> Map.remove bn
