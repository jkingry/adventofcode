...
module Day21 =
                ...
                for roll2 = 1 to 3 do
                    for roll3 = 1 to 3 do
                        ...
                        if nextScore >= 21 then
                            ...
                            // keep playing
                            let (universeCopyOtherWins, universeCopyCurrentWins) =
                                runDiracGame cache (otherPosition, otherScore, nextPosition, nextScore)
    ...
    let run input (output: int -> string -> unit) =
        ...
        let positions =
            input |> splitLine |> Seq.map (splitSpace >> Array.last >> int) |> List.ofSeq
