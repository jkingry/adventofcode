...
module Day08 =
    ...
    let (&&&) (a: Set<'a>) (b: Set<'a>) = Set.intersect a b
    ...
    let parse (input: string) =
        ...
        |> Array.map
            (fun line ->
                let parts =
                    line |> split "|" |> Array.map splitSpace
    ...
    let part1 (text: string) =
        ...
        |> Array.map
            (fun (_, e) ->
                e
                |> Array.filter (fun s -> uniqueNumbers |> Set.contains s.Length))
    ...
    let solve (x: string seq, o: string seq) =
        let l =
            seq { for s in x -> s.Length, set s } |> Map.ofSeq
